...
import { moveupFrontItems, moveFrontItems, deleteFrontItems } from '../actions/frontItemActions';
...
import { IItem, FRONT_CARD_TYPE, FRONT_STICKER_TYPE } from '../states/IStates';
import FrontMenu from './FrontMenu';
import Card from './FrontItemComponents/Card';
import Sticker from './FrontItemComponents/Sticker';
...
class FrontBoard extends React.Component<IProps, IState> {
    ...
    public handleBackClick = (e: React.MouseEvent) => {
        this.setState({
            ...
            showMenu: this.state.isEdit || this.state.isResize ? false : !this.state.showMenu,
            isDrag: false,
            isEdit: false,
            isResize: false,
            targetItemId: '',
    ...
    public handleDown = (e: React.MouseEvent) => {
        ...
        this.props.dispatch(moveupFrontItems(e.currentTarget.id));
        this.setState({
            isEdit: false,
    ...
    public handleUp = (e: React.MouseEvent) => {
        if (this.state.mouseMoveCalled) {
            ...
            if (this.state.canDelete) {
                console.log('deleted');
                this.props.dispatch(deleteFrontItems(this.state.targetItemId));
                this.setState({
                    canDelete: false,
                    isDrag: false,
                    targetItemId: '',
            ...
            if (this.state.isDrag) {
                console.log('dropped');
                this.props.dispatch(moveFrontItems(this.state.targetItemId, this.state.top, this.state.left));
                this.setState({
                    isDrag: false,
                    targetItemId: '',
                });
    ...
    public renderFrontItems = () => {
        return (
            this.props.frontItems.map((item) => (
                 <div
                    id={item.id}
                    key={`front-${item.type}-${item.id}`}
                    onMouseDown={(e) => {
                        e.stopPropagation();
                        this.handleDown(e);
                    }}
